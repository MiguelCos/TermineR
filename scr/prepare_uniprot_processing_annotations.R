...
# PREPARE SYNECHOCYSTIS SP. PCC 6803 UNIPROT ANNOTATION ----- 

## read and load fasta ----

## fasta location

fasta_location <- here("data-raw/prep_annotations/uniprotkb_Synechocystis_sp_taxonomy_1111708_2026_05_27.fasta")
...
if(!file.exists(here("data-raw/prep_annotations/synechocystis_uniprot_features_pkd.rds"))){
  ...
  write_rds(successful_results, 
            file = here("data-raw/prep_annotations/synechocystis_uniprot_features_pkd.rds"))
    
  
} else {
  
  successful_results <- read_rds(here("data-raw/prep_annotations/synechocystis_uniprot_features_pkd.rds"))
...
df_uniprot_synechocystis_features <- purrr::map(successful_results,
                                                safe_feature_to_dataframe)

# extract the $result subelement of each element of the list

df_uniprot_synechocystis_features <- purrr::map(
  df_uniprot_synechocystis_features, 
  ~.x$result)

# exclude any NULL element of the list

df_uniprot_synechocystis_features <- df_uniprot_synechocystis_features %>%
  purrr::keep(~!is.null(.x))

df_synechocystis_features <- bind_rows(df_uniprot_synechocystis_features) %>%
  filter(type %in% mol_processing_feat)

write_tsv(df_synechocystis_features, 
          here("data-raw/prep_annotations/synechocystis_uniprot_processing_features.tsv"))
