# TermineR: extracting information on endogenous proteolytic processingfrom shotgun proteomics data.
...
- `pheatmap`
- `RColorBrewer`
- `arrow` (required for DIA-NN `.parquet` reports)
...
install.packages(c("tidyverse", "here", "janitor", "seqinr", "ggpubr", "ggsignif", "ggrepel", "pheatmap", "RColorBrewer", "arrow"))
...
`report.tsv` or `report.parquet` (precursor-level output) and perform standardization of the
...
- `path_to_file` is the path to the `.tsv` or `.parquet` file containing the DiaNN
  precursor level results (usually called `report.tsv` or `report.parquet`). Parquet
  reports are read with the `arrow` package.
...
- `organism` is the organism used in the search. By default, the value
  ...
  `"mendicato_trucantula"` and `"rhizobium_melitoli"`, `"pig"`, `"arabidopsis"`, `"ecoli"`, `"rat"`, `"yeast"`, `"c_elegans"` and `"synechocystis"` are
