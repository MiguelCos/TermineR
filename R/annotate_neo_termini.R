...
prot2pept2fasta <- left_join(
  ...
  mutate(
    specificity = case_when(
      ...
      sense == "C" & str_detect(aa_before, specificity, negate = TRUE) & !is.na(aa_before) & is.na(aa_after) ~ "semi_Cterm",
      sense == "C" & str_detect(last_aa, specificity, negate = TRUE) & !is.na(aa_after) & is.na(aa_before) ~ "semi_Nterm",
